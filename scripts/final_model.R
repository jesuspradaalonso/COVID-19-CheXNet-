...
############################# GLOBAL PATHS ##################################
path_root <- "/home/";
path_project <- file.path(path_root, "jesusprada", "proyecto_python","x-ray");
path_scripts <- file.path(path_root, "bin", "R");
path_data <- file.path(path_project, "predictions");
#############################  AUXILIARY FUNCTIONS ##################################

#################################### ROC CURVE ########################################
AUC <- function(target, prediction, type = "binary", ret = "value", quiet = FALSE){
  if (type == "multi"){
    roc_curve <- multiclass.roc(target, prediction, levels = levels(target))
  } else {
    roc_curve <- roc(target, prediction, quiet = quiet);
  }
  if (ret == "plot"){
    auc_value <- roc_curve;
  } else {
...
print(table(dat$dataset, dat$survival));
############################# PRE-PROCESSING #############################
# Get target
target <- as.factor(dat$survival);

# Select vars
selected_vars <- c("dataset",  "offset", "sex", "age", "view", "location", "pred")
...
lims<-list(eta_values=c(10^-2, 10^0), gamma_values=c(10^-3, 10^3),max_depth_values=c(10, 10),min_child_weight_values=c(1, 5),
           subsample_values=c(0.5, 1),colsample_bytree_values=c(0.1, 1),num_parallel_tree_values=c(10, 10),
           nrounds_values=c(10, 10), lambda_values = c(0, 0), alpha_values = c(0, 0));
